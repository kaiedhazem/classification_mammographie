...
for i in range(0,int(n)):
    for item in np.array(generateur_validataion[i][1]):
        ...
        j+=1
#acuarcy
print('accuracy du modéle est  :')
print(metrics.accuracy_score(yreel.argmax(axis=1), ypred.argmax(axis=1)))
...
if (len(classe))<8:
        ...
        width= int(len(classe) * .5)
        height= int(len(classe) * .5)
plt.figure(figsize=(width, height))
s.heatmap(matrice, annot=True, vmin=0, fmt='g', cmap='Reds', cbar=False)       
plt.xticks(np.arange(len(classe))+.5, classe, rotation= 90)
plt.yticks(np.arange(len(classe))+.5, classe, rotation=0)
plt.xlabel("Aprés prédiction")
plt.ylabel("Réel")
plt.title("Matrice du Confusion")
plt.show()
